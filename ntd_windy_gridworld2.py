# -*- coding: utf-8 -*-
"""nTD: Windy Gridworld2

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1D0Bq75sWgzxY6uwO28M5J0mGouF_i-s-
"""
...
class windy_gridworld():
  def __init__(self, action_list):
    ...
    #policies
    self.epsilon=0 #make target policy greedy
    self.target_policy, self.behav_policy=self.get_policy()
  ...
  def get_policy(self): #target: greedy w.r.t Q, behav: equiprobable
    target_policy=[]
    behav_policy=[]
    for s_idx, st in enumerate(self.states):
  ...
  def off_policy_sarsa_cov(self): #with control variate
      ...
      while True:
        if step<termin_step:
          ...
          if termination==False:
            ...
            av_actions=self.avail_actions_idx[s_idx_f]
        ...
        if update_step>=0:
          n_step_return=0
          for i in np.arange(min(update_step+self.n, termin_step), update_step, -1): #backwards iterations from t+n until t+1 (for t+1)
            if i==termin_step: #specialize for T-1:T
              n_step_return=ep_reward[termin_step]
            elif i==update_step+self.n: #for t+n (ending of recursion)
              n_step_return=self.action_value[ep_sidx[update_step+self.n],ep_aidx[update_step+self.n]]
            else:
              avg_value=0
              policy=self.target_policy[ep_sidx[i]]
              for a_idx, prob in enumerate(policy):
                avg_value+=prob*self.action_value[ep_sidx[i],a_idx]
              n_step_return=ep_reward[i]+self.gamma*ep_isr[i]*(n_step_return-self.action_value[ep_sidx[i],ep_aidx[i]])+self.gamma*avg_value
  ...
  def tree_backup(self):
    #initialize action value & policy
    self.action_value=torch.zeros(self.len_s, self.len_a)
    self.target_policy, self.behav_policy=self.get_policy()
    for epoch in range(1, self.iter+1):
      ep_sidx=[]
      ...
      s_idx=self.start_idx
      ep_sidx.append(s_idx)
      a_idx=random.choices(self.a_idx_list, self.behav_policy[s_idx])[0]
      ep_aidx.append(a_idx)
      ep_reward.append(0)
      ep_isr.append(1)
      termin_step=100000000
      step=0
      while True:
        if step<termin_step:
          s_idx=ep_sidx[-1]
        ...
        update_step=step-(self.n-1)
        if update_step>=0:
          for i in np.arange(min(update_step+self.n, termin_step),update_step,-1): #iterations for t+1
            if i==termin_step:
              n_step_return=ep_reward[termin_step]
            elif i==update_step+self.n:
              n_step_return=self.action_value[ep_sidx[update_step+self.n], ep_aidx[update_step+self.n]]
            else:
              avg_value=0
              policy=self.target_policy[ep_sidx[i]]
              for a_idx, prob in enumerate(policy):
                  avg_value+=prob*self.action_value[ep_sidx[i],a_idx]
              n_step_return=ep_reward[i]+self.gamma*avg_value+self.gamma*policy[ep_aidx[i]]*(n_step_return-self.action_value[ep_sidx[i],ep_aidx[i]])
          self.action_value[ep_sidx[update_step],ep_aidx[update_step]]+=self.step_size*(n_step_return-self.action_value[ep_sidx[update_step],ep_aidx[update_step]])
          _=self.greedy_update(ep_sidx[update_step])
        if update_step==termin_step-1:
          break
...
#action_value1=gw.off_policy_sarsa_cov()
action_value2=gw.tree_backup()
sl,traj=gw.get_trajectory()

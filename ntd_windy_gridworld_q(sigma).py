# -*- coding: utf-8 -*-
"""nTD: Windy Gridworld Q(sigma)

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1LZtBxseghHwO4pEIj-m229fzROtaWXim
"""
...
class windy_gridworld():
  def __init__(self, action_list):
    ...
    #sigma value
    self.sigma=[0,1] #choice between sampling and expectation
  ...
  def get_policy(self): #target: e-greedy w.r.t Q, behav: equiprobable
    target_policy=[]
    behav_policy=[]
    for s_idx, st in enumerate(self.states):
  ...
  def q_sigma(self):
    for epoch in range(1, self.iter+1):
      ...
      ep_isr.append(1)
      ep_sigma=[]
      ep_sigma.append('null') #no sampling or expecting at first (s,a)
      s_idx=self.start_idx
      ep_sidx.append(s_idx)
      a_idx=random.choices(self.a_idx_list, self.behav_policy[s_idx])[0] #following behavior policy
      ...
      while True:
        if step<termin_step:
          ...
          if termination==False:
            ...
            ep_sigma.append(sigma)
          else:
            termin_step=step+1
        update_step=step-(self.n-1)
        if update_step>=0:
          for t in np.arange(min(update_step+self.n, termin_step), update_step,-1): #iterations for t+1
            if t==termin_step:
              n_step_return=ep_reward[-1]
            elif t==update_step+self.n:
              n_step_return=self.action_value[ep_sidx[update_step+self.n], ep_aidx[update_step+self.n]]
            else:
              avg_value=0
              policy=self.target_policy[ep_sidx[t]]
              for a_idx, prob in enumerate(policy):
                avg_value+=prob*self.action_value[ep_sidx[t],a_idx]
              n_step_return=ep_reward[t]+self.gamma*(ep_sigma[t]*ep_isr[t]+(1-ep_sigma[t])*self.target_policy[ep_sidx[t]][ep_aidx[t]])*(n_step_return-self.action_value[ep_sidx[t],ep_aidx[t]])+self.gamma*avg_value
